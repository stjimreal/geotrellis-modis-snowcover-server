...
import geotrellis.raster.{ArrayMultibandTile, IntConstantNoDataCellType, Raster, Tile}
import geotrellis.spark.SpaceTimeKey.{Boundable, temporalComponent}
...
import geotrellis.spark.{Metadata, MultibandTileLayerRDD, SpaceTimeKey, SpatialKey, TemporalKey, TileLayerMetadata, TileLayerRDD, withTilerMethods}
...
object SpaceTimeIngest {

  case class AccZoom(initVal:Int) extends AccumulatorV2[Int, Int] {
    private var val_ = 0
  ...
  def run(implicit sc:SparkContext, srcPath: String): Unit = {
    ...
    val l = new File(srcPath).listFiles.filter((a: File) => {
    ...
    val accZoom = AccZoom(0)
    sc.register(accZoom, "accZoom")
    val tiled = sc.makeRDD(l).flatMap((f:File) => {
      val fileRdd = sc.hadoopGeoTiffRDD(f.getAbsolutePath)
      val (_, rasterMetaData) = {
        TileLayerMetadata.fromRDD(fileRdd, FloatingLayoutScheme(256))
      }
      val date =  f.getName.substring(0, 7)
      val temporalKey = new TemporalKey(bcTimeFormat.value.parse(date).getTime)
      val layoutScheme = ZoomedLayoutScheme(WebMercator, tileSize = 256)
      val keyRdd = fileRdd
          .tileToLayout(rasterMetaData.cellType, rasterMetaData.layout, Bilinear)
          .repartition(100)

      val (zoom, reprojected) = TileLayerRDD(keyRdd, rasterMetaData).reproject(WebMercator, layoutScheme, Bilinear)
      accZoom.add(zoom)

        reprojected.map(pair =>
        (SpaceTimeKey(pair._1, temporalKey), pair._2)
      ).collect()
    })
...
//    val cogLayer = COGLayer.fromLayerRDD(tiled, accZoom)

    val cogWriter = FileCOGLayerWriter(dstPath)
//    cogWriter.writeCOGLayer()

//    val (_, rasterMetaData) = {
//      TileLayerMetadata.fromRDD(tiled, FloatingLayoutScheme(256))
//    }
